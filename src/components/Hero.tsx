...
const Hero = ({ variant = 'selling' }: HeroProps) => {
  ...
  return (
    <section className="max-w-7xl mx-auto my-8 px-4">
      <div className="grid grid-cols-1 lg:grid-cols-[7fr_3fr] gap-7">
        {/* Left Section */}
        <div className="space-y-6">
          {/* Main Image with Play Button */}
          <div className="relative w-[870px] h-[400px] rounded-2xl overflow-hidden shadow-sm">
            <img
            ...
            {/* Play Button */}
            <button
              className="absolute bottom-4 right-4 bg-white/80 hover:bg-white p-4 rounded-full shadow-lg transition duration-200 flex items-center justify-center"
            >
          ...
          <div className="flex items-center justify-between">
            <h1 className="text-4xl font-bold text-gray-800 leading-tight whitespace-pre-line">
              {content.title}
            </h1>
            <button className="bg-[#2699FB] text-white px-8 py-3 rounded-xl font-semibold hover:bg-blue-700 transition duration-200 shadow-md">
              {content.buttonText}
            </button>
          </div>
        </div>
